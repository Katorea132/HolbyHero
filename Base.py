...
class Base:
    ...
    def name(self, value):
        if value is None:
            ...
            raise ValueError("Make sure the name is no longer \
than 10 characters")
        self.__name = value
    ...
    def race(self, value):
        if value is None:
            raise ValueError("Please, make sure to select 1 of the \
valid races: Human, Elf, Dwarf, Hobbit, Orc or Slime")
        elif type(value) is not str:
            raise TypeError("Make sure the race is a string")
        if value in("Human", "Elf", "Orc", "Hobbit", "Dwarf", "Slime"):
            self.__race = value
        else:
            raise ValueError("Please, make sure to select 1 \
of the valid races: Human, Elf, Dwarf, Hobbit, Orc or Slime")
    ...
    def gender(self, value):
        if value is None:
            raise ValueError("Please, make sure to input a \
valid gender: Male, Female, Apache")
        elif type(value) is not str:
            raise TypeError("Please make sure the gender is a string")
        if value in ("Male", "Female", "Apache"):
            self.__gender = value
        else:
            raise ValueError("Make sure to pick 1 of the \
3 valid genders: Male, Female, Apache")
    ...
    def IncreaseStats(self, **kwargs):
        for value in kwargs.values():
            if type(value) is not int:
                raise ValueError("Make sure to enter a proper value \
for the stat")
    ...
    def Load(self):
        with open("Char", "r", encoding="utf-8") as f:
